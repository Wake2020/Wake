...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: "https://gitee.com/wake326/RandomShareCode/raw/master/JD_Crazy_Joy.json",headers:{
      ...
      try {
        if (err) {
          ...
          $.get({url: `https://gitee.com/wake326/RandomShareCode/raw/master/JD_Crazy_Joy.json`, 'timeout': 10000},(err, resp, data)=>{
          data = JSON.parse(data);})
        } else {
          if (data) {
            console.log(`随机取助力码放到您固定的互助码后面(不影响已有固定互助)`)
